...
EMAIL_CONFIG = {
    ...
    'password': 'bvntpipdhwudbeag',
    'to_email': '2080981057@qq.com'
}

# 钉钉配置（⚠️ 建议改用环境变量 DINGTALK_TOKEN）
DINGTALK_TOKEN = "33ea10c597d41d4275e1c113633bdb22ae644f2c85289b83d68660e67c597cb6"
...
def parse_nginx_log(log_message):
    # 支持带 referrer 和 user_agent 的完整 Nginx 日志
    pattern = r'(\S+) - - \[(.*?)\] "(.*?)" (\d+) (\d+) "(.*?)" "(.*?)"(?: "(.*?)")?'
...
def save_error_log_to_mysql(server_name, raw_message):
    """保存 Nginx error 日志到 error_logs 表"""
    try:
        conn = pymysql.connect(**MYSQL_CONFIG)
        cursor = conn.cursor()
        query = """
        INSERT INTO error_logs (server_name, error_message, created_at)
        VALUES (%s, %s, %s)
        """
        cursor.execute(query, (
            server_name,
            raw_message[:1000],  # 防止超长
            "nginx-error",      # ← 显式指定类型
            datetime.now()
        ))
        conn.commit()
        cursor.close()
        conn.close()
        print(f"[SUCCESS] Error日志已存储: {raw_message[:50]}...")
    except Exception as e:
        print(f"[ERROR] 存储 Error 日志失败: {e}")
        traceback.print_exc()
...
def send_dingtalk_async(msg):
    token = DINGTALK_TOKEN  # 或者用 os.getenv("DINGTALK_TOKEN")
    if not token:
...
def save_access_log_to_mysql(log_record):
    try:
        conn = pymysql.connect(**MYSQL_CONFIG)
        cursor = conn.cursor()
        query = """
...
def process_log_message(message):
    try:
        ...
        # === 关键修复：智能提取 log_type ===
        raw_log_type = (
            log_data.get('log_type') or                     # 顶层字段（如 access/error）
            log_data.get('fields', {}).get('log_type') or   # fields 下（Filebeat fields）
            'access'  # 默认
        ).strip()

        # 标准化为内部类型
        if raw_log_type in ("access", "nginx-access"):
            log_type = "nginx-access"
            log_category = "access"
        elif raw_log_type in ("error", "nginx-error"):
        ...
        print(f"[INFO] 处理 {log_type} 日志 (原始类型: {raw_log_type})")

        if log_category == "access":
            ...
            save_access_log_to_mysql(log_record)
...
用户代理: {parsed['user_agent'][:100]}
            ...
            # 保存到 error_logs 表
            save_error_log_to_mysql(server_name, raw_message)
